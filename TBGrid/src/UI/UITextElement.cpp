...
UITextElement::UITextElement(std::string text, glm::vec2 position, glm::vec2 scale, glm::vec3 color)
	: UIElement(position, glm::vec2{}, scale),
	color(color),
...
UITextElement::UITextElement(std::string text, glm::vec2 position, float scale, glm::vec3 color)
	: UIElement(position, glm::vec2{}, glm::vec2(scale, scale)),
	color(color),
	font(GraphicsResourceManager::getInstance().defaultFont),
	text(text)
{
...
UITextElement::UITextElement(std::string text, glm::vec2 position, glm::vec2 scale, glm::vec3 color, FontAtlas* font)
	: UIElement(position, glm::vec2{}, scale),
		color(color),
		font(font),
		text(text)
...
void UITextElement::checkShader()
{
	if (!textShader)
	{
...
void UITextElement::draw(Shader* shader, const glm::mat4& projection) const
{
	...
	glm::vec2 effectiveScale = computeEffectiveScale();
	...
	for (auto ch : text)
	{
		...
		// position on screen
		float xpos = x + font->characters[ch].bl * effectiveScale.x;
		float ypos = y - (font->characters[ch].bh - font->characters[ch].bt) * effectiveScale.y;
		float w = font->characters[ch].bw * effectiveScale.x;
		float h = font->characters[ch].bh * effectiveScale.y;
		...
		x += font->characters[ch].ax * effectiveScale.x;
	}
}
