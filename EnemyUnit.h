#pragma once

#include <vector>
#include "BoxCollider.h"
#include "GameObject.h"
#include "GraphicsResourceManager.h"
#include "LevelGrid.h"
#include "StaticMesh.h"
#include "Transform.h"

namespace Enemy
{
	const static float MOVEMENT_SPEED = 2.0f;	//Movement speed in tiles/s (which is also units/s as tiles are 1 unit wide)
	const static glm::vec3 CELL_OFFSET{ 0.5f, 0.1f, 0.5f };	//In order for an enemy unit to appear normal, this is the offset added to the position of a level tile to get their position within it
}

class EnemyUnit : public GameObject
{
private:
	std::vector<glm::vec3> movementRoute;
	int movementTargetIndex = 0;		//the current location on the route the unit is aiming for
public:
	EnemyUnit();
	~EnemyUnit() {}
	bool hasAction = false;			//Has this unit been assigned an action that they are now in the process of completing?
	bool actionAvailable = false;	//Is this unit free to select or be assigned an action?
	void update(const float deltaTime);
	void assignMovementAction(std::vector<glm::vec3> movementRoute);
	bool processAction(const float deltaTime);	//make progress on any action this unit is involved in. Returns true when the action completes
	//AI Stuff
	void determineAction(const LevelGrid& levelInfo);
};